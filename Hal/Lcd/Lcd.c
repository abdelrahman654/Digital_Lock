...
void H_Lcd_WriteFloat(f32 Local_f32_FloatNum,u8 Local_u8_DigitsAfterPoint)
{
	u8 Local_u8_Arr [10] = {0};
	s8 Local_s8_Counter  = 0;
	u8 Local_u8_Digit    = 0;
	u8 Local_s32_IntegerPart = 0;
	f32 Local_f32_FloatPart = 0.0;
	Local_s32_IntegerPart = (signed int)Local_f32_FloatNum;
	Local_f32_FloatPart = Local_f32_FloatNum - (signed int)Local_f32_FloatNum;
	if(Local_s32_IntegerPart == 0)
	{
		H_Lcd_WriteCharacter('0');
	}
	else
	{
		...
		{
			H_Lcd_WriteCharacter('-');
			Local_s32_IntegerPart = Local_s32_IntegerPart * -1;
		}
		while(Local_s32_IntegerPart != 0)
		{
			Local_u8_Digit = Local_s32_IntegerPart % 10;
			Local_u8_Arr[Local_s8_Counter] = Local_u8_Digit;
			Local_s8_Counter++;
			Local_s32_IntegerPart = Local_s32_IntegerPart / 10;
		}
		Local_s8_Counter--;
		while(Local_s8_Counter >= 0)
		{
	...
	H_Lcd_WriteCharacter('.');
	Local_f32_FloatPart = Local_f32_FloatPart * pow(10,Local_u8_DigitsAfterPoint);
	H_Lcd_WriteNumber(Local_f32_FloatPart);
}
void H_Lcd_Clr(void)
{
	H_Lcd_WriteCommand(LCD_CLEAR);
